...
for file in $(ls shorted/*.csv); do
  ...
  fileName=${fileName/.csv/}
  # xsv sample 1000 $file | csvsql -I -z 9261072 --tables $fileName --no-create --db mysql+mysqlconnector://box:xxx@localhost:3306/CB --insert
  echo "\"csvsql -I --snifflimit 0 -z 9261072 --no-create --db mysql+mysqlconnector://box:xxx@localhost:3306/CB --insert $file\"" >> pump_CB.list
...
# source "/home/steph/workspace.neugelb/GPP/openapi-schema-dev/csv/lazy_load.list"
...
# LOAD DATA LOCAL INFILE '/var/lib/mysql-files/AUSZUG.csv' INTO TABLE CB.AUSZUG IGNORE 1 LINES;
